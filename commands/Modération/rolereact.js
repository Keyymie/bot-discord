...
module.exports.run = async (bot, message, args, config) => {
  ...
  if(args[8]) {if(message.guild.roles.find(x => x.name === args[8])) {roles.push(args[8]);reactions.push("8⃣")} else {message.channel.send(`Le rôle ${args[8]} n'existe pas.`);return}}
  if(args[9]) {message.channel.send("Vous ne pouvez pas ajouter plus de 8 rôles."); return}
...
module.exports.help = {
  name: "crm",
  description: 'Permet de créer un message permettant de récupérer un rôle en cliquant sur une simple réaction.',
  usage: `.crm`
}
